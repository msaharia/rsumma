...
#'
#' @examples
#' temporalplot('data_raw/benchmarking/PNW_3L_1H.nc', 1, 'figures/summaoutput_3L_1H.pdf')
temporalplot <- function(ncname, hruIndex, outfile){
  ...
  #Forcings
  pptrate <- ncvar_get(summaout, 'pptrate')[hruIndex,]*3600 #Convert from kgm-2s-1 to mm/hour
  airtemp <- ncvar_get(summaout, 'airtemp')[hruIndex,]-272.15 #Convert to Centigrade
  windspd <- ncvar_get(summaout, 'windspd')[hruIndex,] #m/s
  scalarRainfall <- ncvar_get(summaout, 'scalarRainfall')[hruIndex,]*3600 # "computed rainfall rate (instant)" # Convert from kgm-2s-1 to mm/hour
  SWRadAtm <- ncvar_get(summaout, 'SWRadAtm')[hruIndex,]
  LWRadAtm <- ncvar_get(summaout, 'LWRadAtm')[hruIndex,]

  #Others
  scalarSurfaceTemp <- ncvar_get(summaout, 'scalarSurfaceTemp')[hruIndex,] -272.15 #Convert to Centigrade

  #Canopy
  scalarCanopyIce <- ncvar_get(summaout, 'scalarCanopyIce')[hruIndex,] #mass of ice on the vegetation canopy (instant), kg m-2
  scalarCanopyLiq <- ncvar_get(summaout, 'scalarCanopyLiq')[hruIndex,]
  scalarSnowDepth <- ncvar_get(summaout, 'scalarSnowDepth')[hruIndex,]*1000 #From m to mm
  scalarBelowCanopySolar <- ncvar_get(summaout, 'scalarBelowCanopySolar')[hruIndex,]
  scalarWindspdCanopyBottom <- ncvar_get(summaout, 'scalarWindspdCanopyBottom')[hruIndex,]*3600000 #m/s to mm/h
  scalarSenHeatGround <- ncvar_get(summaout, 'scalarSenHeatGround')[hruIndex,]
  scalarSenHeatTotal <- ncvar_get(summaout, 'scalarSenHeatTotal')[hruIndex,]
  scalarLatHeatGround <- ncvar_get(summaout, 'scalarLatHeatGround')[hruIndex,]
  scalarLatHeatTotal <- ncvar_get(summaout, 'scalarLatHeatTotal')[hruIndex,]

  #Soil
  scalarSnowSublimation <- ncvar_get(summaout, 'scalarSnowSublimation')[hruIndex,]*3600 #Convert from kgm-2s-1 to mm/hour
  scalarThroughfallSnow <- ncvar_get(summaout, 'scalarThroughfallSnow')[hruIndex,]*3600 #Convert from kgm-2s-1 to mm/hour
  scalarThroughfallRain <- ncvar_get(summaout, 'scalarThroughfallRain')[hruIndex,]*3600 #Convert from kgm-2s-1 to mm/hour
  scalarRainPlusMelt <- ncvar_get(summaout, 'scalarRainPlusMelt')[hruIndex,]*3600000 #m/s to mm/h
  scalarInfiltration <- ncvar_get(summaout, 'scalarInfiltration')[hruIndex,]*3600000 #m/s to mm/h
  scalarExfiltration <- ncvar_get(summaout, 'scalarExfiltration')[hruIndex,]*3600000 #m/s to mm/h
  scalarSWE <- ncvar_get(summaout, 'scalarSWE')[hruIndex,]*3600000 #m/s to mm/h
  scalarSurfaceRunoff <- ncvar_get(summaout, 'scalarSurfaceRunoff')[hruIndex,]*3600000 #m/s to mm/h
  scalarSoilDrainage <- ncvar_get(summaout, 'scalarSoilDrainage')[hruIndex,]*3600000 #m/s to mm/h

  # Layer information
  nLayers <- ncvar_get(summaout, 'nLayers')[hruIndex,] #Total of layers at every time step
  nSnow <- ncvar_get(summaout, 'nSnow')[hruIndex,] #Number of snow layers at every time step
  nSoil <- ncvar_get(summaout, 'nSoil')[hruIndex,] #Number of soil layers at every time step
  nLayers <- ncvar_get(summaout, 'nLayers')[hruIndex,] #Number of snow layers at every time step

  ########################################################################
  # Soil Moisture
  ########################################################################
  #Dimensions
  midTotoStartIndex <- ncvar_get(summaout, 'midTotoStartIndex')[hruIndex,] #start index of the midToto (snow+soil) vector for a given timestep
  ...
  mLayerVolFracLiq <- ncvar_get(summaout, 'mLayerVolFracLiq')[hruIndex,] #Soil Moisture
  ...
  ########################################################################
  mLayerVolFracWat <- ncvar_get(summaout, 'mLayerVolFracWat')[hruIndex,] #Soil Moisture
  ...
  ########################################################################
  mLayerVolFracIce <- ncvar_get(summaout, 'mLayerVolFracIce')[hruIndex,] #Soil Moisture
  ...
  ########################################################################
  mLayerMatricHead <- ncvar_get(summaout, 'mLayerMatricHead')[hruIndex,] #Soil Moisture
  ...
  ########################################################################
  mLayerTemp <- ncvar_get(summaout, 'mLayerTemp')[hruIndex,]-272.15 #Soil Moisture
...
# temporalplot('data_raw/benchmarking/PNW_8L_3H.nc', 1, 'figures/summaoutput_8l_3H.pdf')
